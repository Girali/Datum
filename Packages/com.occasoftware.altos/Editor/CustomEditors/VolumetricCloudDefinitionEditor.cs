    ...
    public class VolumetricCloudDefinitionEditor : UnityEditor.Editor
    {
        ...
        void DrawBasicShadows()
        {
            ...
            if (basicParams.castShadowsEnabled.boolValue)
            {
                ...
                EditorGUILayout.PropertyField(basicParams.shadowResolution, new GUIContent("Shadow Resolution"));
                EditorGUILayout.PropertyField(basicParams.shadowStrength, new GUIContent("Shadow Strength"));
                EditorGUILayout.PropertyField(
                    basicParams.shadowDistance,
                    new GUIContent(
                        "Shadow Distance (m)",
                        "Radius around the camera in which cloud shadows will be rendered. Clamped to the camera's far clip plane and the cloud fade distance."
                    )
                );
            }
        }
