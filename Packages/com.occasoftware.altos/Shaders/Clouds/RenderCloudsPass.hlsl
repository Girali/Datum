...
#define EPSILON_LARGE 0.01
//static half EPSILON_LARGE = 0.01;
...
half GetWeathermap(StaticMaterialData materialData, RayData rayData, half cloudinessAtPoint)
{
    ...
    else
    {
        ...
        weathermapSample = GetLayeredPerlinNoise(_WEATHERMAP_OCTAVES, weathermapUV, _WEATHERMAP_GAIN, _WEATHERMAP_LACUNARITY);
    }
	
    return Remap01(1.0 - cloudinessAtPoint, 1.0, weathermapSample);
}

half GetCloudDensityByHeight(StaticMaterialData materialData, RayData rayData, half height01)
{
    /* USE A MAP TO MODULATE CLOUD BASE + CAP HEIGHTS:
    float2 weathermapUV = GetWeathermapUV(rayData.rayPosition, materialData.mainCameraOrigin, false);
    weathermapUV *= _CLOUD_WEATHERMAP_SCALE * 2.0;
    float weathermapSample = GetLayeredPerlinNoise(3, weathermapUV, 0.5, 2.0);
    weathermapSample -= 0.5;
    return saturate(_CLOUD_DENSITY_CURVE_TEX.SampleLevel(linear_clamp_sampler, float2(height01 + weathermapSample, 0), 0).r);
    */
...
OSLightingData GetLightingDataVolumetric(StaticMaterialData materialData, RayData rayData, AtmosphereData atmosData, int mip)
{
	...
	*/
    ...
    int sampleCount = 4;
#define LIGHTING_SAMPLE_COUNT 4
	...
	*/
    ...
    half lightingDistanceToSample = materialData.lightingDistance;
    ...
    half sGain = 0.3;
    half3 densityAdj = 0;
    for (int i = 1; i <= LIGHTING_SAMPLE_COUNT; i++)
    {
        half lightSample = half(i) / half(LIGHTING_SAMPLE_COUNT + 1);
        lightSample = lightSample * lightSample; // Simplified to pow2
        ...
        if (weather > EPSILON_LARGE)
        {
            half densityAtHeight = GetCloudDensityByHeight(materialData, rayData, height01);
...
half GetAmbientDensity(StaticMaterialData materialData, RayData rayData, AtmosphereData atmosData, half weather, half cloudinessAtPoint, int mip, int cheapAmbientLighting)
{
    ...
    half step = atmosData.atmosThickness * 0.2;
	
    rayData.rayPosition += float3(0, 1, 0) * step;
	
    half density = GetCloudDensityByHeight(materialData, rayData, height01);
...
{
    half atmosphericAttenuation = 1.0 - rcp(exp(accumulatedDepth * atmosphereDensity));
...
void SampleHighAltitudeClouds(RayData rayData, StaticMaterialData materialData, AtmosphereData atmosData, float highAltitudeHitDistance, inout float alpha, inout float3 cloudColor)
{
    ...
    for (int a = 0; a <= MAX_STEPS; a++)
    {
        ...
        if (valueAtPoint > EPSILON_LARGE)
        {
            ...
            alpha *= transmittance;
            half3 invSampleExtinction = rcp(sampleExtinction);
...
half _CLOUD_EXTINCTION_COEFFICIENT;
half _CLOUD_COVERAGE;
half _CLOUD_STEP_COUNT;
half _CLOUD_LAYER_HEIGHT;
half _CLOUD_LAYER_THICKNESS;
half _CLOUD_FADE_DIST;
half3 _CLOUD_BASE_SCALE;
half _CLOUD_BLUE_NOISE_STRENGTH;
half _CLOUD_DETAIL1_STRENGTH;
half3 _CLOUD_BASE_TIMESCALE;
half3 _CLOUD_DETAIL1_TIMESCALE;
half3 _CLOUD_DETAIL1_SCALE;
half _CLOUD_FOG_POWER;
half _CLOUD_MAX_LIGHTING_DIST;
half _CLOUD_PLANET_RADIUS;
half _CLOUD_CURL_SCALE;
half _CLOUD_CURL_STRENGTH;
half _CLOUD_CURL_TIMESCALE;
half _CLOUD_AMBIENT_EXPOSURE;
half _CLOUD_DISTANT_COVERAGE_START_DEPTH;
half _CLOUD_DISTANT_CLOUD_COVERAGE;
half2 _CLOUD_DETAIL1_HEIGHT_REMAP;
bool _CLOUD_DETAIL1_INVERT;
half _CLOUD_HEIGHT_DENSITY_INFLUENCE;
half _CLOUD_COVERAGE_DENSITY_INFLUENCE;

// High alt
half2 _CLOUD_HIGHALT_OFFSET1;
half2 _CLOUD_HIGHALT_OFFSET2;
half2 _CLOUD_HIGHALT_OFFSET3;
half2 _CLOUD_HIGHALT_SCALE1;
half2 _CLOUD_HIGHALT_SCALE2;
half2 _CLOUD_HIGHALT_SCALE3;
half _CLOUD_HIGHALT_COVERAGE;
half _CLOUD_HIGHALT_INFLUENCE1;
half _CLOUD_HIGHALT_INFLUENCE2;
half _CLOUD_HIGHALT_INFLUENCE3;
half _CLOUD_HIGHALT_EXTINCTION;

bool _CLOUD_DEPTH_CULL_ON;
half _CLOUD_SCATTERING_AMPGAIN;
half _CLOUD_SCATTERING_DENSITYGAIN;
int _CLOUD_SCATTERING_OCTAVES;
half _CLOUD_HGFORWARD;
half _CLOUD_HGBACK;
half _CLOUD_HGBLEND;
half _CLOUD_HGSTRENGTH;
...
float4 SampleClouds(float2 UV)
{
    ...
    materialData.rayOrigin = GetCameraPosition();
    ...
    materialData.ambientExposure = _CLOUD_AMBIENT_EXPOSURE;
    ...
    materialData.renderLocal = _CLOUD_DEPTH_CULL_ON;
    ...
    AtmosphereData atmosData = New_AtmosphereData(_CLOUD_LAYER_THICKNESS * _CONVERT_KM_TO_M, _CLOUD_LAYER_HEIGHT * _CONVERT_KM_TO_M, _CLOUD_FADE_DIST * _CONVERT_KM_TO_M, _CLOUD_DISTANT_CLOUD_COVERAGE, _CLOUD_DISTANT_COVERAGE_START_DEPTH * _CONVERT_KM_TO_M);
    ...
    half invNumSteps = rcp(_CLOUD_STEP_COUNT);
    ...
    if (sampleLowAltitudeClouds)
    {
        ...
        for (int i = 1; i <= int(_CLOUD_STEP_COUNT); i++)
        {
            ...
            if (weather > EPSILON_LARGE)
            {
                ...
                density = GetCloudDensityByHeight(materialData, rayData, height01);
				
                valueAtPoint = GetCloudShapeVolumetric(materialData, rayData, atmosData, weather, density, height01, cloudinessAtPoint, mip, true);
            }
			
            if (valueAtPoint > EPSILON_LARGE && isFirstSample)
            {
                i -= 1;
            ...
            float stepSize = rayData.stepSize;
            if (shortStepCounter > 0)
            {
                stepSize = rayData.stepSize * 0.2;
            }
            ...
            if (valueAtPoint > EPSILON_LARGE)
            {
                ...
                half transmittance = exp(-sampleExtinction.r * stepSize);
                ...
                if (sampleAmbientLighting)
                {
                    ...
                    half ambDensity = GetAmbientDensity(materialData, rayData, atmosData, weather, cloudinessAtPoint, mip, _CheapAmbientLighting);
            ...
            if (shortStepCounter > 0)
            {
                rayData.rayDepth += rayData.stepSize * 0.2;
                shortStepCounter -= 1;
            }
            else
            {
	...
	*/
    ...
    cloudData = float4(cloudColor, alpha);
