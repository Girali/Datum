    ...
    internal sealed class AltosRenderFeature : ScriptableRendererFeature
    {
        ...
        private class SkyRenderPass : ScriptableRenderPass
        {
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                KeywordHandler.DisableKeywords(cmd);
                ...
                if (skyDirector.skyDefinition != null)
                {
                    ...
                    cmd.SetRenderTarget(source, renderingData.cameraData.renderer.cameraDepthTarget);
                }
                #endregion
        ...
        private class VolumetricCloudsRenderPass : ScriptableRenderPass
        {
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                void RenderClouds(CommandBuffer cmd, RenderTargetIdentifier source)
                {
                    ...
                    cmd.SetRenderTarget(depthTex.Identifier());
                    cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, ditherDepth);
                    ...
                    cmd.SetRenderTarget(cloudTarget.Identifier());
                    cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, cloudRenderMaterial);
                }
                void UpscaleClouds(CommandBuffer cmd, out RenderTargetIdentifier taaInput)
                {
                    ...
                    if (skyDirector.cloudDefinition.renderScaleSelection != RenderScaleSelection.Full)
                    {
                        if (skyDirector.cloudDefinition.renderScaleSelection == RenderScaleSelection.Quarter)
                        {
                            ...
                            cmd.SetGlobalTexture("_ScreenTexture", upscaleInput);
                            cmd.SetRenderTarget(upscaleQuarterRes.Identifier());
                            cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, upscale);

                            cmd.SetGlobalFloat(CloudShaderParamHandler.ShaderParams._UPSCALE_SOURCE_RENDER_SCALE, 0.5f);
                            cmd.SetGlobalTexture("_ScreenTexture", upscaleQuarterRes.Identifier());
                            cmd.SetRenderTarget(upscaleHalfRes.Identifier());
                            cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, upscale);
                ...
                void TemporalAntiAliasing(CommandBuffer cmd, RenderingData renderingData, RenderTargetIdentifier taaInput)
                {
                    ...
                    cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, cloudTaa);
                ...
                void Merge(CommandBuffer cmd, RenderTargetIdentifier source)
                {
                    ...
                    cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, merge);
        ...
        private class CloudShadowsRenderPass : ScriptableRenderPass
        {
            ...
            {
                if (shadowTaa == null)
                    shadowTaa = CoreUtils.CreateEngineMaterial(skyDirector.data.shaders.integrateCloudShadows);
            ...
            public override void Configure(CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor)
            {
                ...
                cmd.GetTemporaryRT(shadowsToScreenTarget.id, rtDescriptor, FilterMode.Bilinear);
                ...
                void ConfigureShadows(CommandBuffer cmd, RenderTextureDescriptor descriptor)
                {
                    if (skyDirector.cloudDefinition.castShadowsEnabled)
                    {
                        ...
                        if (cloudShadowHistory == null)
                        {
                            ...
                            cloudShadowHistory.filterMode = FilterMode.Bilinear;
                ...
                {
                    cmd.GetTemporaryRT(screenShadowsTarget.id, descriptor, FilterMode.Bilinear);
                }
            }
            ...
            bool GetSunData(out SunData sunData)
            {
                ...
                if (skyDirector != null && skyDirector.Sun != null)
                {
                    sunData.direction = skyDirector.Sun.GetForward();
                    sunData.forward = skyDirector.Sun.GetChild().forward;
                    sunData.right = skyDirector.Sun.GetChild().right;
                    sunData.up = skyDirector.Sun.GetChild().up;
                    return true;
                }
                else
                {
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                void RenderShadows(CommandBuffer cmd, RenderingData renderingData, RenderTargetIdentifier source, SunData sunData)
                {
                    ...
                    int halfWidth = (int)
                        Mathf.Min(
                            skyDirector.cloudDefinition.shadowDistance,
                            Mathf.Min(renderingData.cameraData.camera.farClipPlane, skyDirector.cloudDefinition.cloudFadeDistance * 1000)
                        );
                    ...
                    cmd.SetRenderTarget(cloudShadowsTempTarget.Identifier());
                    cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, cloudRenderMaterial);

                    cmd.SetGlobalTexture(CloudShaderParamHandler.ShaderParams.Shadows._CLOUD_SHADOW_PREVIOUS_HISTORY, cloudShadowHistory);
                    cmd.SetGlobalTexture(
                        CloudShaderParamHandler.ShaderParams.Shadows._CLOUD_SHADOW_CURRENT_FRAME,
                        cloudShadowsTempTarget.Identifier()
                    );
                    shadowTaa.SetFloat(CloudShaderParamHandler.ShaderParams.Shadows._IntegrationRate, 0.03f);
                    cmd.SetRenderTarget(cloudShadowsTarget.Identifier());
                    cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, shadowTaa);
                ...
                void DrawScreenSpaceShadows(CommandBuffer cmd, RenderTargetIdentifier source)
                {
                    ...
                    cmd.SetGlobalTexture("_ScreenTexture", source);
                    cmd.SetRenderTarget(screenShadowsTarget.Identifier());
                    cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, screenShadows);
                }
                void RenderToScreen(CommandBuffer cmd, RenderTargetIdentifier source)
                {
                    cmd.SetGlobalTexture("_ScreenTexture", source);
                    cmd.SetRenderTarget(shadowsToScreenTarget.Identifier());
                    cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, shadowsToScreen);
        ...
        private class AtmosphereBlendingPass : ScriptableRenderPass
        {
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                cmd.SetGlobalTexture("_ScreenTexture", source);
                cmd.SetRenderTarget(blendingTarget.Identifier());
                cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, blendingMaterial);
        ...
        private static class KeywordHandler
        {
        ...
        private class CloudShaderParamHandler
        {
            ...
            public static void SetCloudMaterialSettings(CloudDefinition d, Material cloudRenderMaterial)
            {
                ...
                cloudRenderMaterial.SetFloat(ShaderParams.CloudData._WEATHERMAP_OCTAVES, d.weathermapOctaves);
                cloudRenderMaterial.SetFloat(ShaderParams.CloudData._WEATHERMAP_GAIN, d.weathermapGain);
                cloudRenderMaterial.SetFloat(ShaderParams.CloudData._WEATHERMAP_LACUNARITY, d.weathermapLacunarity);
            }
        }
    }
}
